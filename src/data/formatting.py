"""
src/data/formatting.py
======================
Contains system prompts, templates, and formatting functions for lead conversations.
"""

from __future__ import annotations

# System prompt used in training chat conversations
SYSTEM_PROMPT = (
    "You are a real estate lead qualification assistant for Anywhere Real Estate "
    "(operating brands including Coldwell Banker, Century 21, ERA, Sotheby's International Realty, "
    "Better Homes and Gardens Real Estate, and Corcoran). Given a lead conversation, output a JSON "
    "with: intent (buy/rent/invest/browse), urgency (hot/warm/cold), budget_range, timeline_months, "
    "preferred_locality, property_type, brand_fit (which Anywhere brand best matches the lead), "
    "next_action (agent_call/schedule_showing/follow_up_email/nurture_sequence/drop), score (0–100), "
    "and reasoning."
)

# Template prompt injected to drive synthetic lead data generation from Gemini REST API
LEAD_QUALIFICATION_PROMPT = """You are a synthetic training data generator for a real estate lead qualification AI fine-tuned for Anywhere Real Estate (brands: Coldwell Banker, Century 21, ERA, Sotheby's International Realty, Better Homes and Gardens Real Estate, Corcoran).
...
congent on current home sale, no pre-approval yet, rate uncertainty causing hesitation,
down payment funds not yet liquid, spouse or partner not yet aligned, timeline genuinely vague,
waiting on a life event (job offer, inheritance, divorce settlement)
...
def format_to_conversations(user_text: str, assistant_text: str) -> dict:
    """Format single QA examples into Chat conversations scheme."""
    return {
        "conversations": [
